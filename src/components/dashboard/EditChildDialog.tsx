...
import { Child, SUBJECTS, GRADE_LEVELS } from "@/types";
...
export function EditChildDialog({
  ...
  const { updateChild, loading } = useChildrenStore();
  ...
  useEffect(() => {
    if (child) {
      form.reset({
        ...
        strugglingWith: child.strugglingWith || "",
      });
    }
  }, [child, form]);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Chỉnh sửa thông tin {child.name}</DialogTitle>
          <DialogDescription>
            Cập nhật thông tin chi tiết về con em để tìm gia sư phù hợp hơn.
          </DialogDescription>
        </DialogHeader>
